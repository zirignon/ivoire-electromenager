// Supabase pour le site (commandes)
...
// =======================
// Enregistrer une commande dans Supabase (table orders)
async function saveOrderToSupabase(order) {
    try {
        if (typeof supabaseClient === 'undefined') {
            console.error('supabaseClient non défini (saveOrderToSupabase).');
            return false;
        }
        ...
        const { error } = await supabaseClient
            ...
            .insert({
                ...
                payment_method: order.paymentMethod,   // 'livraison', 'OMCIV2', 'MOMOCI', 'FLOOZ', 'WAVECI', 'CARD'
                payment_label: order.paymentLabel,     // texte lisible (Paiement à la livraison, Orange Money, etc.)
                payment_status: 'pending',             // sera mis à 'paid' par la fonction PaiementPro
                status: 'Nouvelle',
                total: order.total
            });
        ...
        console.error('Erreur saveOrderToSupabase :', err);
...
if (!saved) {
    return; // on arrête si la sauvegarde a échoué
}

// 2) Lancer le bon flux de paiement
if (isOnlinePayment) {
...
// Cache local des commandes chargées
let adminOrdersCache = [];
window.currentOrderStatusFilter = window.currentOrderStatusFilter || 'all';

// Adapter une ligne de la table "orders" à l'affichage admin
function mapDatabaseOrderToAdminOrder(order) {
    return {
        id: Number(order.id),
...
// Charger les commandes depuis Supabase (avec articles déjà agrégés côté Supabase, si tu as une vue, sinon on peut ignorer items)
async function fetchOrdersFromDatabase() {
    if (typeof supabaseClient === 'undefined') {
        console.error('supabaseClient non défini (admin).');
        return [];
    }
    ...
    try {
        ...
        // - OU paiement en ligne (OMCIV2, MOMOCI, FLOOZ, WAVECI, CARD) avec payment_status = 'paid'
        const onlineMethods = ['OMCIV2', 'MOMOCI', 'FLOOZ', 'WAVECI', 'CARD'];
        ...
        adminOrdersCache = filtered.map(mapDatabaseOrderToAdminOrder);
        return adminOrdersCache;
    } catch (error) {
        console.error('Erreur générale chargement commandes:', error);
        return [];
    }
}

// Afficher les commandes dans #ordersList
...
// Filtrer les commandes (recherche + statut + visibilité)
window.filterOrders = async function () {
    ...
    if (window.currentOrderStatusFilter !== 'all') {
        orders = orders.filter(order => (order.status || 'Nouvelle') === window.currentOrderStatusFilter);
    }

    // Filtrer par texte (id, nom client, téléphone, etc.)
    if (searchTerm) {
...
// Mettre à jour le statut d'une commande (colonne "status" de la table orders)
async function updateOrderStatus(orderId, status) {
    try {
        if (typeof supabaseClient === 'undefined') {
            console.error('supabaseClient non défini (updateOrderStatus).');
            return;
        }
