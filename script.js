...
function displayProducts(productsToDisplay) {
    ...
    productsToDisplay.forEach((product, index) => {
        ...
        const outOfStockBadge = isOutOfStock
            ? `<span class="product-out-of-stock-badge">Rupture de stock</span>`
            : '';

        card.innerHTML = `
            <img src="${escapeAttribute(safeImage)}" alt="${escapeAttribute(product.name)}" class="product-image${isOutOfStock ? ' product-image--out-of-stock' : ''}">
            ${promoBadge}
            ${outOfStockBadge}
            <div class="product-info">
                ...
                <div class="product-footer">
                    ...
                    <button class="btn-add" type="button" onclick="openProductModal(${Number(product.id)})"${isOutOfStock ? ' disabled style="opacity:0.5;cursor:not-allowed;"' : ''}>
                        ${isOutOfStock ? 'Indisponible' : 'Voir plus'}
                    </button>
                </div>
            </div>
        `;
...
async function deleteCustomProduct(productId) {
    const confirmDelete = confirm('Voulez-vous vraiment supprimer ce produit ? Cette action est irréversible.');
    ...
    try {
        ...
        if (reviewsError) {
            console.error('Erreur suppression avis:', reviewsError);
        }
    ...
    if (countEl) {
        if (currentSearchTerm) {
            ...
            countEl.textContent = filtered.length === 0
                ? 'Aucun produit trouvé pour "' + currentSearchTerm + '"'
                : filtered.length + ' produit' + (filtered.length > 1 ? 's' : '') + ' trouvé' + (filtered.length > 1 ? 's' : '') + ' pour "' + currentSearchTerm + '"';
        } else {
...
async function openProductModal(productId) {
    ...
    if (allImages.length > 1) {
        ...
        allImages.forEach(function(imgObj, index) {
    ...
    // Gestion rupture de stock dans le modal
...
let activePromo = null; // { code, discount_percent, id }
...
async function applyPromoCode() {
    ...
    // Vérifier que le client ne l'a pas déjà utilisé (nécessite son email)
    ...
    if (email) {
        ...
        if (usage) {
            msg.style.color = '#e74c3c';
            msg.textContent = 'Vous avez déjà utilisé ce code promo.';
            return;
        }
    }

    activePromo = { id: promo.id, code: promo.code, discount_percent: promo.discount_percent };
    msg.style.color = '#16a34a';
    msg.textContent = 'Code appliqué ! -' + promo.discount_percent + '% sur votre commande.';
    updateCartUI();
}
...
function getCartTotal() {
    ...
    if (!activePromo) return { subtotal, discount: 0, total: subtotal };
    const discount = Math.round(subtotal * activePromo.discount_percent / 100);
...
function updateCartUI() {
    ...
    if (discountRow) {
        if (discount > 0 && activePromo) {
            ...
            if (discountLabel) discountLabel.textContent = 'Réduction (' + activePromo.discount_percent + '%) : ';
...
function openCheckoutForm() {
    ...
    const { subtotal, discount, total } = getCartTotal();
    let checkoutTotalHtml = formatPrice(total);
    if (discount > 0 && activePromo) {
        checkoutTotalHtml = `<span style="text-decoration:line-through;color:#aaa;font-size:0.85rem;">${formatPrice(subtotal)}</span> <span style="color:#16a34a;">${formatPrice(total)}</span> <span style="font-size:0.8rem;color:#16a34a;">(-${activePromo.discount_percent}%)</span>`;
    }
    checkoutTotal.innerHTML = checkoutTotalHtml;
...
async function saveOrderToDatabase(orderData) {
    ...
    if (activePromo && orderData.customer.email) {
        try {
            await supabaseClient.from('promo_usage').insert({
                promo_code_id: activePromo.id,
                customer_email: orderData.customer.email.toLowerCase(),
                order_id: orderId
            });
        } catch (err) {
            console.error('Erreur enregistrement usage promo:', err);
        }
        activePromo = null;
...
async function processCheckout(e) {
    ...
    // Calculer le total de la commande (avec remise si code promo actif).
