...
function toggleReviewForm() {
    ...
    if (!isVisible) {
        ...
        const ref = document.getElementById('reviewRef');
        ...
        _currentRating = 0;
        document.querySelectorAll('.star-btn').forEach(function(s) { s.style.opacity = '0.35'; });
    }
}
...
async function loadReviews(productId) {
    ...
    if (error || !reviews || reviews.length === 0) {
        ...
        document.getElementById('modalRating').textContent = '⭐ Aucun avis';
        return;
    }
    ...
    document.getElementById('modalRating').textContent = stars + ' ' + avgStr + '/5 (' + reviews.length + ' avis)';
...
async function submitReview() {
    const ref     = document.getElementById('reviewRef').value.trim().toUpperCase();
    const email   = document.getElementById('reviewEmail').value.trim().toLowerCase();
    ...
    // Vérifier que la commande existe avec cet email et contient ce produit
    ...
    // Vérifier que cette commande contient le produit en cours
...
async function openProductModal(productId) {
    ...
    // Galerie miniatures (uniquement les images sans couleur spécifique = communes)
