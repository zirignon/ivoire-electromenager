...
// En cas d'échec ou d'abandon, le panier est conservé intact pour que
// le client puisse réessayer sans tout reconstituer.
async function checkPendingPaymentOnReturn() {
    ...
    // Petite fonction utilitaire pour interroger le statut réel en base.
    ...
    // Le webhook de Paiement Pro peut arriver avec un léger délai après la
    // redirection de retour. Si le statut est encore "pending", on retente
    // une seconde fois après une courte attente avant de conclure.
    if (status === 'pending') {
    ...
    if (status === 'success') {
        ...
        // On garde le panier intact pour permettre un nouvel essai.
        localStorage.removeItem('ivoirePendingPayment');
        showNotification('❌ Le paiement a échoué. Votre panier a été conservé.', 'error');
    } else {
        // Statut toujours indéterminé (pending persistant ou commande
        // introuvable) : on garde le marqueur pour réessayer la prochaine
        // fois que le client revient sur le site, et on ne touche pas au panier.
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    // Si l'utilisateur revient d'un paiement en ligne (PAYEMENT PRO), vérifie
    // le vrai statut en base et vide le panier seulement si le paiement a réussi.
...
async function saveOrderToDatabase(orderData) {
    ...
    // 4. Envoyer l'email "commande reçue" au client (best-effort : si ça
    // échoue, on ne bloque pas le tunnel de commande, on logue juste l'erreur).
    try {
...
async function startPaiementProPayment(order) {
    try {
        ...
        // Sauvegarde un marqueur local pour pouvoir vérifier le statut du
        // paiement au retour sur le site (voir checkPendingPaymentOnReturn).
