...
async function startPaiementProPayment(order) {
    try {
        ...
        /*
            IMPORTANT :
            GitHub Pages est un site statique.
            Il ne peut pas recevoir une notification serveur PAYEMENT PRO.

            Plus tard, pour confirmer automatiquement un paiement,
            on devra utiliser une Supabase Edge Function comme notificationURL.
        */
...
window.filterOrders = async function() {
    ...
    // Masquer les commandes PAYEMENT PRO non payées.
    // Les paiements à la livraison restent visibles.
    const onlinePaymentMethods = ['OMCIV2', 'MOMOCI', 'FLOOZ', 'WAVECI', 'CARD'];
    ...
    orders = orders.filter(order => {
        ...
        // Paiement à la livraison ou anciennes commandes : visible.
        if (!isOnlinePayment) {
            return true;
        }

        // Paiement en ligne : visible seulement si payé.
