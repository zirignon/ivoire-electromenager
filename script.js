...
async function openEditProductModal(productId) {
    ...
    // Charger les photos existantes depuis product_images
    ...
    if (gallery) {
        ...
        const imgs = (existingImages && existingImages.length > 0)
            ? existingImages
            : (product.image ? [{ id: null, url: product.image, position: 0 }] : []);
        ...
        imgs.forEach(function(img) {
            ...
            div.innerHTML =
                ...
                (img.id ? '<button type="button" onclick="deleteProductImage(\'' + img.id + '\', this)" title="Supprimer">✕</button>' : '');
    ...
    try {
        ...
        if (newFiles.length > 0) {
            ...
            for (let i = 0; i < newFiles.length; i++) {
                ...
                // Mettre à jour l'image principale si c'est la première photo ajoutée
...
async function deleteProductImage(imageId, btn) {
    ...
    const thumb = btn.closest('.edit-gallery-thumb');
...
async function openProductModal(productId) {
    ...
    // Charger les images depuis product_images
    ...
    const images = (productImages && productImages.length > 0)
        ? productImages.map(function(img) { return img.url; })
        : (currentProduct.image ? [getSafeImageSrc(currentProduct.image)] : []);
...
function addToCartFromModal() {
    ...
    addToCartWithVariant(currentProduct.id, quantity, window._selectedVariant || null);
    window._selectedVariant = null;
    closeModal();
    showNotification('Produit ajouté au panier !');
}

function addToCartWithVariant(productId, quantity, variant) {
    const id = Number(productId);
    ...
    const cartProduct = variant
        ? Object.assign({}, product, { selectedVariant: variant, cartKey: id + '-' + variant })
